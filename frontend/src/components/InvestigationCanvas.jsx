...
import { ChevronDown, ChevronRight, Clock, Check, X, ShieldAlert, GitBranch, User, Lock, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: '1px solid #1F2937', background: '#0d1117' }}>
      <button
        ...
        <span className="h-meta font-bold flex-shrink-0 w-5 h-5 rounded flex items-center justify-center"
          style={{ background: '#1F2937', color: '#6B7280' }}>
          {step.step}
        </span>
        <span className="flex-1 h-title-sm" style={{ color: '#CBD5E1' }}>{step.action}</span>
        <div className="flex items-center gap-2 flex-shrink-0">
          {step.mitre && (
            <span className="h-meta px-1.5 py-0.5 rounded"
              style={{ background: '#D84C7F11', color: '#D84C7F', border: '1px solid #D84C7F22' }}>
              {step.mitre}
            </span>
          )}
          <span className="h-body font-bold" style={{ color: confColor }}>{step.confidence}%</span>
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t" style={{ borderColor: '#1F2937' }}>
          <div className="flex items-center gap-1.5 mt-2 mb-1.5">
            ...
            <span className="h-meta" style={{ color: '#3B6FE3' }}>{step.agent}</span>
          </div>
          <p className="h-body leading-relaxed" style={{ color: '#9CA3AF' }}>
            {step.finding}
          </p>
    ...
    <div className="rounded-lg p-3 animate-slide-in-up"
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="h-meta font-bold" style={{ color: sevColor }}>{item.severity}</span>
          <span className="h-meta" style={{ color: '#6B7280' }}>{item.timestamp}</span>
        </div>
        <span className="h-meta" style={{ color: '#4B5563' }}>{item.id}</span>
      </div>
      <p className="h-title mb-0.5" style={{ color: '#E2E8F0' }}>
        {item.action}
      </p>
      <p className="h-body mb-1" style={{ color: '#93C5FD' }}>Asset: {item.asset}</p>
      <p className="h-meta italic mb-3" style={{ color: '#6B7280' }}>⚠ {item.risk}</p>
    ...
    <div className="w-80 border-l flex flex-col flex-shrink-0 animate-slide-in-right" 
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="flex items-center justify-between mb-2">
          <span className="h-label" style={{ color: '#6B7280' }}>Open Investigations</span>
          <span className="h-meta" style={{ color: '#4B5563' }}>{investigations.length} ACTIVE</span>
        </div>

        {investigations.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 opacity-50 space-y-3">
            <ShieldCheck size={24} color="#6B7280" />
            <p className="h-body text-center" style={{ color: '#6B7280' }}>No active investigations automatically created.</p>
          </div>
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg border border-transparent hover:border-[#3B6FE340] hover:bg-[#3B6FE305] transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="h-meta font-bold" style={{ color: '#4B5563' }}>{inv.id}</span>
                  <span className="h-meta px-1.5 py-0.5 rounded uppercase" 
                        style={{ background: '#1F2937', color: '#9CA3AF' }}>{inv.status}</span>
                </div>
                <h4 className="h-title truncate">{inv.title}</h4>
                <div className="flex items-center gap-2 mt-1">
                  <span className="h-meta" style={{ color: '#6B7280' }}>Agent: {inv.assigned_agent}</span>
                </div>
              </div>
            </div>
          </div>

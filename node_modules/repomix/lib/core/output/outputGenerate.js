...
export const generateOutput = async (rootDirs, config, processedFiles, allFilePaths, gitDiffResult = undefined, gitLogResult = undefined, filePathsByRoot, deps = {
    ...
    const outputGeneratorContext = await deps.buildOutputGeneratorContext(rootDirs, config, allFilePaths, sortedProcessedFiles, gitDiffResult, gitLogResult, filePathsByRoot);
...
export const buildOutputGeneratorContext = async (rootDirs, config, allFilePaths, processedFiles, gitDiffResult = undefined, gitLogResult = undefined, filePathsByRoot, deps = {
    listDirectories,
    listFiles,
    searchFiles,
}) => {
    ...
    if (shouldUseFullTree) {
        ...
        try {
            const merged = (await Promise.all(rootDirs.map((rootDir) => deps.searchFiles(rootDir, config)))).reduce((acc, curr) => ({

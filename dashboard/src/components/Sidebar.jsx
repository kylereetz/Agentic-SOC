...
const NAV_ITEMS = [
  ...
  { icon: Bot,          label: 'Agents',           badge: '12', color: '#D84C7F' },
              ...
              {!collapsed && (
                <>
                  <span
                    ...
                    onMouseEnter={e => { if (!active) e.target.style.color = '#9CA3AF'; }}
                    onMouseLeave={e => { if (!active) e.target.style.color = '#6B7280'; }}>
                    {label}
                  </span>
                  {displayBadge && (
                    <span
                      className="ml-auto text-xs px-1.5 py-0.5 rounded-full terminal transition-all"
                      style={{
                        background: `${color}22`,
                        color: color,
                        fontSize: '10px',
      ...
      {!collapsed && (
        <div className="px-3 py-3 border-t" style={{ borderColor: '#1F2937' }}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs terminal" style={{ color: '#4B5563' }}>AGENT MESH</span>
          </div>
          {/* Mini node grid */}
          <div className="grid grid-cols-6 gap-1">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i}
                ...
                style={{
                  background: i < 10 ? '#88C057' : '#EF4444',
                  boxShadow: i < 10 ? '0 0 4px rgba(136,192,87,0.5)' : '0 0 4px rgba(239,68,68,0.5)',
                  animationDelay: `${i * 120}ms`,
                }} />
            ))}
          </div>
          <p className="text-xs terminal mt-1.5" style={{ color: '#4B5563' }}>12/12 NODES ONLINE</p>
          ...
          {pendingCount > 0 && (
            <div className="mt-2 flex items-center gap-1.5 px-2 py-1 rounded"
              ...
              <span className="text-xs terminal" style={{ color: '#E5A862' }}>{pendingCount} PENDING</span>
            </div>
          )}
        </div>
      )}

...
def test_triage_spoofing_prevention():
    ...
    engine.rules = [
        {
            ...
            "pattern": {"event_type": "vuln_exploit"},
        }
    ]
    ...
    if alert1.severity == "WARNING":
        print(
            "[SUCCESS] Corroboration mandate successfully capped spoofed UDP alert to WARNING."
        )
    else:
    ...
    if alert2.severity == "CRITICAL":
        print(
            "[SUCCESS] State/Telemetry mechanism permitted verified TCP connection to remain CRITICAL."
        )
    else:
    ...
    if alert3.severity == "CRITICAL":
        print(
            "[SUCCESS] State/Telemetry mechanism permitted EDR correlated alert to remain CRITICAL."
        )
    else:

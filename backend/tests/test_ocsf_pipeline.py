"""
Automated Test Suite: Test Ocsf Pipeline.
...
async def test_pipeline():
    ...
    if ocsf_it:
        ...
        if alert:
            logger.info(
                f"Triage successfully classified IT OCSF Alert: {alert.rule_name} (Severity: {alert.severity})"
            )
        else:
            logger.info(
                "Triage correctly bypassed the IT event (no malicious rules hit)."
            )
    else:
    ...
    if ocsf_ot:
        ...
        if alert:
            logger.info(
                f"Triage successfully classified OT OCSF Alert: {alert.rule_name} (Severity: {alert.severity})"
            )

"""
Automated Test Suite: Test Rare Event Model.
    ...
    for alert in [alert3, alert4]:
        if alert and "Dormant" in alert.get("rule_name", ""):
            if "USER::admin_service" in alert.get("description", ""):
                ...
                logger.info(
                    f"Success: Dormant User awakening caught! Severity: {alert.get('severity')}"
                )
            if "IP::10.0.0.51" in alert.get("description", ""):
                found_ip_alert = True
                logger.info(
                    f"Success: Dormant IP awakening caught! Severity: {alert.get('severity')}"
                )

    if found_user_alert and found_ip_alert:
        logger.info(
            "Historian Test completed successfully. Both User and IP entities tracked."
        )
    else:
        logger.error(
            "Historian test failed to trigger threshold alerts on all entities."
        )

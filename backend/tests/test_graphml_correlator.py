...
async def test_subgraph_clustering():
    ...
    if found_campaign:
        logger.info(f"Success: The GraphML Engine clustered the nodes and emitted the Campaign Incident! Details: {details}")
    else:
        logger.error("Failed: No GraphML Campaign was emitted. Check temporal clustering math.")
        
    # Manually trigger export so we can verify the file
    correlator._persist_state()
    
    if os.path.exists(GRAPHML_OUTPUT_PATH):
        logger.info(f"Verified: Subgraph mathematically exported to {GRAPHML_OUTPUT_PATH}.")
    else:

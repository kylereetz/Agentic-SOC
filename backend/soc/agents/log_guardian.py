...
class LogExtraction(BaseModel):
    ...
    extracted_regex_pattern: str = Field(description="A Python regex string with named capture groups (?P<ip>, ?P<meaning>, ?P<codes>) that would extract these fields deterministically next time.")
...
class LogGuardianAgent:
    def __init__(self):
        ...
        self.learned_decoders_file = get_soc_path("soc", "agents", "learned_decoders.json")
    ...
    def _save_decoder(self, pattern_str: str):
        try:
            ...
            logger.info(f"[+] LLM successfully wrote new deterministic rule: {pattern_str}")
        except Exception as e:
        ...
        if "user" in raw_data.lower() and "login" in raw_data.lower():
             ...
             for rule in self.learned_rules:
                 ...
                 if m:
                     ...
                     unmapped_dict = {
                         "inferred_meaning": group_dict.get("meaning", "regex_fallback_match"),
                         "proprietary_codes": group_dict.get("codes", ""),
                         "original_source": source,
                         "raw_event": raw_data
                     }
                     ocsf_obj = OCSFProprietaryOT(
                        metadata=OCSFMetadata(normalization_type="custom_OT_inferred_cached"),
                        time=time.time(),
    ...
    async def run(self):
        ...
        logger.info("[SQ] Log-Guardian Pydantic AI Normalizer started (Self-Healing Active).")

...
if not logger.handlers:
    ...
    formatter = logging.Formatter("%(asctime)s - %(levelname)s - RCA Hunter - %(message)s")
...
class HunterAgent:
    ...
    async def _process_hunting_event(self, event: Dict[str, Any]):
        ...
        if event_type == "new_intel_lead":
            ...
            if matches:
                ...
                logger.info(f"[VQ] Hunt complete for {hypothesis}: No matches found in history.")

    async def _search_historical_data(self, keywords: List[str], days: int) -> List[Dict[str, Any]]:
        """
        ...
        channels = [d for d in os.listdir(BUS_ROOT) if os.path.isdir(os.path.join(BUS_ROOT, d))]
                ...
                try:
                    with open(file_path, "r") as f:
                        ...
                        if event_ts:
                            ...
                            except: pass # Ignore bad formats
                            
                        # Keyword match
                        if any(kw.lower() in data_str for kw in keywords):
                            matches.append({
                                "asset_id": data.get("asset_id", data.get("source_ip", "Unknown")),
    ...
    async def _report_hunt_findings(self, hypothesis: str, matches: List[Dict[str, Any]], lead_event: Dict[str, Any]):
        """[EQ] Pattern Backtracking: Linking new leads to old events."""
        logger.warning(f"[EQ] HUNT SUCCESS: Found {len(matches)} historical matches for hypothesis: {hypothesis}")

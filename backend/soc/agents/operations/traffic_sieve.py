...
if not logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - RCA TrafficSieve - %(message)s"
    )
...
class TrafficSieveAgent:
    ...
    async def run(self):
        ...
        logger.info(
            f"[SQ] Traffic-Sieve Graph Specialist started. Learning Window: {LEARNING_PERIOD_SECONDS}s"
        )
    ...
    async def _analyze_graph_flow(self, flow: Dict[str, Any]):
        ...
        if self.graph.has_edge(src_ip, dst_ip):
            ...
            m2 = edge_data.get("m2_bytes", 0.0) + (bytes_sent - old_mean) * (
                bytes_sent - new_mean
            )
            ...
            if count > 10:
                ...
                if (
                    std_dev > 0
                    and bytes_sent > (old_mean + 3 * std_dev)
                    and (bytes_sent - old_mean) > 1000000
                ):
            ...
            edge_data["bytes_transfer"] = (
                edge_data.get("bytes_transfer", 0) + bytes_sent
            )
            ...
            byte_bin = (
                ...
                if bytes_sent < 100
                    ...
                    if bytes_sent < 1000
                        ...
                        "1KB-10KB"
                        if bytes_sent < 10000
                        else ("10KB-100KB" if bytes_sent < 100000 else ">100KB")
                    )
                )
            )
            self.graph.add_edge(
        ...
        if not is_learning_mode:
            await self._run_detection_algorithms(
                src_ip,
                dst_ip,
                dst_port,
                novel_edge,
    ...
    async def _run_detection_algorithms(
        self,
        src: str,
        dst: str,
        port: int,
        novel_edge: bool,
        volumetric_spike: bool,
        v_details: dict,
        entropy_spike: bool,
        e_details: dict,
        proto: str,

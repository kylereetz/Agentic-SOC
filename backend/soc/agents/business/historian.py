...
if not logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - RCA Historian - %(message)s"
    )
...
class HistorianAgent:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Synchronized with doctrine: {get_soc_path('ethos', 'ethos_flyway_historian.md')}"
        )

    def _init_db(self):
        self.conn = ServiceMesh.connect_db(
            client_identity="historian",
    ...
    async def run(self):
        ...
        logger.info(
            f"[SQ] FLYWAY-HISTORIAN awakened. Silence Threshold set to: {THRESHOLD_SECONDS} seconds."
        )
    ...
    async def _process_event(self, event: Dict[str, Any]):
        ...
        for entity_id, entity_type in entities:
            ...
            cursor = self.conn.execute(
                "SELECT last_seen FROM entity_activity WHERE entity_id = ?",
                (entity_id,),
            )
            row = cursor.fetchone()
            ...
            if row is None:
                ...
                if silence_duration > THRESHOLD_SECONDS:
                    logger.warning(
                        f"[IQ] Threshold of Silence broken for {entity_id}. Dormant for {silence_duration} seconds."
                    )
                    ...
                    alert = {
                        ...
                        "severity": (
                            "CRITICAL" if entity_type == "user_account" else "WARNING"
                        ),
                ...
                self.conn.execute(
                    "UPDATE entity_activity SET last_seen = ? WHERE entity_id = ?",
                    (now, entity_id),
                )

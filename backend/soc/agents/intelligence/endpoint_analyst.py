...
if not logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - RCA Endpoint Analyst - %(message)s"
    )
...
class EndpointAnalystAgent:
    ...
    def _init_db(self):
        ...
        self.conn = ServiceMesh.connect_db(
            client_identity="endpoint_analyst",
            db_path=db_path,
    ...
    def _recalculate_clusters_sync(self):
        ...
        for idx, u1 in enumerate(users):
            ...
            for u2 in users[idx + 1 :]:
                ...
                sim = self._jaccard_similarity(
                    self.user_vectors[u1], self.user_vectors[u2]
                )
        ...
        logger.info(
            f"[IQ] Rebalanced Peer Groups. Unified {len(users)} users into {len(self.clusters)} mathematical clusters."
        )
    ...
    async def _process_event(self, event: Dict[str, Any]):
        ...
        if user and process_name:
            ...
            if process_name not in self.user_vectors[user]:
                ...
                try:
                    self.conn.execute(
                        "INSERT OR IGNORE INTO user_processes (username, process) VALUES (?, ?)",
                        (user, process_name),
                    )
                    self.conn.commit()
            ...
            if is_new_process:
                ...
                if my_cluster and len(my_cluster) >= 2:
                    ...
                    for peer in my_cluster:
                        if peer != user and process_name in self.user_vectors.get(
                            peer, set()
                        ):
                    ...
                    if peer_group_dev:
                        reason = (
                            f"PEER GROUP DEVIATION: User '{user}' executed '{process_name}', which has "
                        ...
                        await self._escalate(
                            ...
                            reason,
                            0.95,
                            rule_id="PEER_GROUP_DEVIATION",
                            override_name="Behavioral Peer Deviation",
                        )
                        return
        ...
        if event_type == "sysmon" and event.get("eid") == 1:
            if "powershell" in event.get("process_name", "").lower():
                if (
                    ...
                    or "hidden" in command_line
                ):
                    await self._escalate(
                        event,
                        "WARNING",
                        "Heuristic: Obfuscated PowerShell execution detected natively.",
                        0.9,
                    )
                    return
        ...
        if event_type == "sysmon" and event.get("eid") == 8:
            await self._escalate(
                event,
                "CRITICAL",
                "Heuristic: Remote Thread Creation (Code Injection) detected natively.",
                0.95,
            )
            return
        ...
        if command_line:
            ...
            try:
                ...
                if res.get("malicious"):
                    await self._escalate(
                        event,
                        res.get("severity", "WARNING"),
                        f"LLM Match: {res.get('reason')}",
                        0.8,
                    )
            except Exception as e:
    ...
    async def _escalate(
        ...
        confidence: float,
        rule_id: str = "EP_001",
        override_name: str = "Endpoint Behavioral Anomaly",
    ):

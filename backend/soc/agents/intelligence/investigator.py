...
class InvestigationConclusion(BaseModel):
    ...
    severity: str = Field(description="The final severity assessment (INFO, WARNING, CRITICAL)")
    ...
    summary: str = Field(description="Direct, concise summary of the confirmed threat activity")
    hypothesis: str = Field(description="The primary working hypothesis of what occurred")
    containment_strategy: Optional[str] = Field(description="Drafted containment action steps")
...
class InvestigatorTools:
    ...
    def collect_forensics(self, target_ip: str, artifact_type: str, investigation_id: str) -> str:
...
class InvestigatorAgent:
    ...
    def _register_tools(self):
        @self.ai_agent.tool
        async def query_siem(ctx: RunContext[AgentDeps], source_ip: str, time_range: str = "-1h") -> str:
            """Query SIEM for events from a source IP."""
            res = ctx.deps.tools.query_siem(source_ip, time_range)
            self._publish_step(ctx.deps, "ACTION", f"Querying SIEM for {source_ip}", tool="query_siem")
        ...
        async def check_threat_intel(ctx: RunContext[AgentDeps], indicator: str) -> str:
            ...
            self._publish_step(ctx.deps, "ACTION", f"TI Lookup: {indicator}", tool="check_threat_intel")
            self._publish_step(ctx.deps, "OBSERVATION", res)
            return res

        @self.ai_agent.tool
        async def collect_forensics(ctx: RunContext[AgentDeps], target_ip: str, artifact_type: str = "MEMORY") -> str:
            """Collect forensics (MEMORY, PCAP) from a host."""
            res = ctx.deps.tools.collect_forensics(target_ip, artifact_type, ctx.deps.investigation_id)
            self._publish_step(ctx.deps, "ACTION", f"Forensics {artifact_type} collection: {target_ip}", tool="collect_forensics")
            self._publish_step(ctx.deps, "OBSERVATION", res)
            return res

        @self.ai_agent.tool
        async def draft_containment(ctx: RunContext[AgentDeps], strategy: str, target_ip: str) -> str:
            """Draft a containment action (ISOLATE, QUARANTINE)."""
            res = ctx.deps.tools.draft_containment(strategy, target_ip)
            self._publish_step(ctx.deps, "ACTION", f"Drafted {strategy} on {target_ip}", tool="draft_containment")
            self._publish_step(ctx.deps, "OBSERVATION", res)
            return res

    def _publish_step(self, deps: AgentDeps, type: str, content: str, tool: Optional[str] = None):
        step = ReasoningStep(
    ...
    async def _investigate(self, alert: Dict[str, Any], investigation_id: str):
        ...
        try:
            ...
            logger.info(f"[{investigation_id}] Investigation complete: {conclusion.severity}")
            
            # Final Conclusion Broadcast
            self._publish_step(deps, "CONCLUSION", f"{conclusion.severity}: {conclusion.summary}")

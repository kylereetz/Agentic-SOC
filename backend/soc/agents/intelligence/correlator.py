...
class CorrelatorAgent:
    ...
    def __init__(self):
        ...
        self.promotion_threshold = (
            100  # Cumulative score required to promote a subgraph
        )
    ...
    def _parse_severity(self, alert: Dict[str, Any]) -> int:
        ...
        if str(sev).upper() == "CRITICAL":
            return 80
        if str(sev).upper() == "HIGH":
            return 50
        if str(sev).upper() == "MEDIUM":
            return 20
        return 10
    ...
    async def _process_stream(self):
        ...
        while self.is_running:
            ...
            if alert:
                ...
                rule_name = alert.get(
                    "rule_name", alert.get("event_type", "Unknown Event")
                )
                ...
                for node_id, node_type in entities:
                    if not self.graph.has_node(node_id):
                        self.graph.add_node(
                            node_id, type=node_type, risk_score=0, event_count=0
                        )
                ...
                for i in range(len(entities)):
                    for j in range(i + 1, len(entities)):
                        ...
                        if not self.graph.has_edge(u, v):
                            self.graph.add_edge(
                                u, v, weight=1, last_seen=now, events=[]
                            )
    ...
    async def _evaluate_clusters(self):
        ...
        for cluster_nodes in clusters:
            ...
            cumulative_risk = sum(
                nx.get_node_attributes(subgraph, "risk_score").values()
            )
    ...
    async def _promote_to_incident(self, subgraph: nx.Graph, score: int):
        ...
        for u, v, data in edges:
            for event in data.get("events", []):
                e_lower = event.lower()
                if "scan" in e_lower or "recon" in e_lower:
                    stages.add("RECON")
                elif "smb" in e_lower or "login" in e_lower or "lateral" in e_lower:
                    stages.add("LATERAL")
                elif "exfil" in e_lower or "upload" in e_lower:
                    stages.add("EXFIL")

        active_stage = "UNKNOWN"
        if "RECON" in stages:
            active_stage = "RECON"
        if "LATERAL" in stages:
            active_stage = "LATERAL"
        if "EXFIL" in stages:
            active_stage = "EXFIL"

        logger.info(
            f"[VQ] PROMOTING GRAPHML CAMPAIGN! Nodes: {len(nodes)}, Cum. Risk: {score}, Stage: {active_stage}"
        )
    ...
    async def _cleanup_loop(self):
        ...
        while self.is_running:
            ...
            if len(stale_edges) > 0 or len(stale_nodes) > 0:
                logger.info(
                    f"[EQ] Temporal Decay activated: Pruned {len(stale_edges)} edges & {len(stale_nodes)} orphaned nodes."
                )

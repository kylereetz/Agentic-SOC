...
class CryptoAgilityManager:
    ...
    @classmethod
    def validate_negotiated_cipher(
        cls, negotiated_cipher: str, assurance_level: str = "HIGH_ASSURANCE"
    ) -> bool:
        """
...
class ServiceMesh:
    ...
    def connect_db(
        ...
        if client_identity not in VALID_MTLS_CERTIFICATES:
            logger.critical(
                f"[DENY-IDENTITY] Unauthorized mTLS attempt to Data Tier by: {client_identity}"
            )
        ...
        if (
            not negotiated_cipher
            or not CryptoAgilityManager.validate_negotiated_cipher(
                negotiated_cipher, assurance_level="HIGH_ASSURANCE"
            )
        ):
            logger.critical(
                f"[DENY-CRYPTOGRAPHY] {client_identity} attempted DB connection with legacy/unapproved Cipher: {negotiated_cipher}"
            )
            raise mTLSAuthenticationError(
        ...
        logger.debug(
            f"[ALLOW] mTLS and Cryptographic Curve verified for {client_identity}. Granting DB connection."
        )
...
class CloudStorageGateway:
    ...
    def put_object(client_role: str, file_path: str, data: str):
        ...
        if client_role not in CloudStorageGateway.ALLOWED_S3_WRITERS:
            logger.critical(
                f"[DENY] IAM violation detected! {client_role} attempted S3 PUT."
            )
        ...
        logger.info(
            f"[ALLOW] IAM verified for {client_role}. Writing object to {file_path}"
        )

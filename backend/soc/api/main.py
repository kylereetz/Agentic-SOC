...
if "api_secret_key" not in vault_data:
    logging.getLogger(__name__).info(
        "Generating new secure API secret key and storing in Vault."
    )
    vault_data["api_secret_key"] = os.environ.get(
        "SOC_API_SECRET_KEY", secrets.token_hex(32)
    )
...
AGENT_ROSTER = [
    {
        "id": "SYRINX-ORCHESTRATOR",
        "role": "Orchestrator",
        "color": "#D84C7F",
        "status": "ACTIVE",
        "model_tier": "Reasoning (Q2.5 7B)",
        "task": "Routing pending alerts",
        "success": 98,
    },
    {
        "id": "QUILL-TRIAGE",
        ...
        "status": "ACTIVE",
        "model_tier": "Reasoning (Q2.5 7B)",
        "task": "Classifying anomalies",
        "success": 92,
    },
    {
        "id": "QUILL-CORRELATOR",
        "role": "CorrelatorAgent",
        "color": "#E5A862",
        "status": "IDLE",
        "model_tier": "Fast (Q2.5 3B)",
        "task": "Awaiting linkages",
        "success": 94,
    },
    {
        "id": "QUILL-LIBRARIAN",
...
async def get_status(user: User = Depends(get_current_user)):
    ...
    return {"scout_status": "operational", "bus_sizes": sizes}
...
async def get_alerts(user: User = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error reading alerts from DB: {exc}"
        )
...
async def get_pending_actions(user: User = Depends(check_role(["admin", "analyst"]))):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error reading pending actions: {exc}"
        )
...
async def approve_action(
    action_id: str, req: ApprovalRequest, user: User = Depends(check_role(["admin"]))
):
    ...
    if not action_id or not re.match(r"^[a-zA-Z0-9_.-]+$", action_id):
        logger.warning(
            f"Invalid action_id format received: '{action_id}' (User: {user.username})"
        )
    ...
    if not success:
        logger.error(
            f"Action approval failed: Invalid signature or ID {action_id} not found. (User: {user.username})"
        )
        raise HTTPException(
            status_code=403,
            detail="Approval rejected: Invalid signature or missing action.",
        )
...
async def get_agent_telemetry(agent_id: str, user: User = Depends(get_current_user)):
    ...
    return {
        ...
        "recent_events": [
            {
                "timestamp": (
                    datetime.now(timezone.utc) - timedelta(minutes=random.randint(1, 5))
                ).isoformat()
                + "Z",
                "type": "ACTION",
...
async def toggle_kill_switch(user: User = Depends(check_role(["admin"]))):
    ...
    logger.warning(
        "[KILL-SWITCH] Emergency kill switch %s by operator '%s'", state, user.username
    )
    return {
...
async def chitchat(req: ChitChatRequest, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            if os.path.exists(cases_dir):
                ...
                if case_files:
                    ...
                    case_files.sort(
                        key=lambda x: os.path.getmtime(os.path.join(cases_dir, x)),
                        reverse=True,
                    )
        ...
        messages = [{"role": "system", "content": f"{system_logic}{context_str}"}]
...
async def get_patch_drafts(user: User = Depends(check_role(["admin", "analyst"]))):
    ...
    fix_files = sorted(
        [f for f in os.listdir(_DRAFTS_DIR) if f.endswith(("_fix.sh", "_fix.ps1"))]
    )
...
async def get_forensics_evidence(
    user: User = Depends(check_role(["admin", "analyst"]))
):
                ...
                results.append(
                    {
                        ...
                        "threat_score": ev.get("iq_analysis", {}).get(
                            "threat_score", 0
                        ),
                        "patterns": ev.get("iq_analysis", {}).get(
                            "pattern_detection", []
                        ),
...
async def get_chain_of_custody(user: User = Depends(check_role(["admin", "analyst"]))):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Could not read chain of custody log"
        )
...
async def get_malware_reports(user: User = Depends(check_role(["admin", "analyst"]))):
    ...
    for case_id, artifact in memory_artifacts:
        ...
        has_cobalt = any(
            "shellcode" in str(p).lower() or "pe_header" in str(p).lower()
            for p in patterns
        )
        ...
        report = {
            ...
            "static_analysis": {
                "suspicious_strings": (
                    ...
                    if has_cobalt
                    else ["IEX (New-Object Net.WebClient).Download"]
                ),
                "imported_apis": (
                    [
                        "VirtualAllocEx",
                    ...
                    if has_cobalt
                    else ["Net.WebClient", "System.Reflection.Assembly"]
                ),
                "sections": (
                    [
                ...
                [
                    {
                        ...
                        "pid": 9912,
                        "detail": f"svchost.exe spawned from {target_ip}",
                    },
                    {
                        "t": "+0.3s",
                        "event": "Memory allocation",
                        "pid": 9912,
                        "detail": "VirtualAllocEx(EXECUTE_READWRITE, 0x1400)",
                    },
                    {
                        "t": "+0.4s",
                        "event": "Code injection detected",
                        "pid": 9912,
                        "detail": "WriteProcessMemory → CreateRemoteThread",
                    },
                    {
                        "t": "+1.1s",
                        "event": "Network beacon",
                        "pid": 9912,
                        "detail": "TCP 203.0.113.45:443 — interval 60s",
                    },
                    {
                        "t": "+2.2s",
                        "event": "Credential access attempt",
                        "pid": 9912,
                        "detail": "sekurlsa::logonpasswords via LSASS handle",
                    },
                    {
                        "t": "+4.5s",
                        "event": "Persistence established",
                        "pid": rng.randint(8000, 12000),
                        "detail": "Run key: HKLM\\\\Run\\\\RCA_Update",
                    },
                ]
                if has_cobalt
                else [
                    {
                        ...
                        "pid": rng.randint(3000, 8000),
                        "detail": "powershell.exe -ExecutionPolicy Bypass",
                    },
                    {
                        "t": "+0.5s",
                        "event": "Download cradle",
                        "pid": rng.randint(3000, 8000),
                        "detail": "IEX WebClient.DownloadString from C2",
                    },
                    {
                        "t": "+1.2s",
                        "event": "Dropper unpacked",
                        "pid": rng.randint(3000, 8000),
                        "detail": "Assembly.Load(byte[]) in-memory",
                    },
                ]
            ),
            "network_iocs": (

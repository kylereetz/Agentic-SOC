"""
Sentinel Hive Benchmark & Hardening Audit Suite.
Executes system-wide validation of multi-agent correlation, RAG search, WAL persistence, and dispatch flow.
"""
...
async def run_hive_benchmark():
    ...
    try:
        ...
        campaign_cases = [
            c
            for c in manager.active_cases.values()
            if "Campaign" in c.alert_details.get("rule_name", "")
        ]
        if campaign_cases:
            ...
            audit_results["critiques"].append(
                "Correlator promotion to Manager failed to trigger Case creation."
            )
        ...
        if os.path.exists(
            manager.conn_path
            if hasattr(manager, "conn_path")
            else "soc/reports/incidents/cases/soc_cases.db"
        ):
        ...
        if (
            len(matches) <= 2
        ):  # Slack + PD for initial opening, none for subsequent saves
            print(
                f"  [PASS] Dispatch suppressed redundant noise ({len(matches)} notifications sent)."
            )
            audit_results["scores"]["noise_reduction"] = 9.5
        else:
            print(
                f"  [FAIL] Dispatch sent too many notifications ({len(matches)}) for a single case."
            )

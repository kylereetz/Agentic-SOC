...
def _start_polling_agent(module_name: str, class_name: str, log_message: str):
    ...
    while not shutdown_event.is_set():
        ...
        if count > 0:
            logger.info(f"Cycle complete: processed {count} items.")
...
AGENT_REGISTRY = {
    ...
    "specialist-id": lambda: _start_specialist("IdentityAccessAnalyst"),
    "specialist-fix": lambda: _start_specialist("RemediationAnalyst"),
    "specialist-lab": lambda: _start_specialist("MalwarePathologist"),
...
def start(
    ...
    if agent in AGENT_REGISTRY:
        try:
            ...
            logger.info("Received KeyboardInterrupt directly. Shutting down...")
    else:

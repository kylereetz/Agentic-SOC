...
if not logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - RCA Red Team - %(message)s"
    )
...
class RedTeamAgent:
    ...
    def __init__(self):
        ...
        logger.warning(
            f"Synchronized with doctrine: {get_soc_path('ethos', 'ethos_quill_red.md')}"
        )
    ...
    async def run(self):
        ...
        logger.warning(
            "[EQ] QUILL-RED Team Auditor online. Commencing continuous evaluation operations."
        )
        ...
        while self.is_running:
            ...
            logger.info(
                f"Stealth phase. Waiting {delay} seconds before next injection sequence..."
            )
            await asyncio.sleep(delay)

            payload = self._generate_synthetic_payload()
            logger.warning(
                f"Injecting simulated attack [{payload['protocol']}] from {payload['ip']} into discovery stream."
            )

...
class ForensicsAgent:
    ...
    def _log_coc_event(self, event_type: str, evidence_id: str, case_id: str, target: str, collector: str = "FORENSICS-01"):
    ...
    def collect_evidence(self, case_id: str, target_ip: str, artifact_type: str) -> Dict[str, Any]:
        """
        ...
        if artifact_type == "MEMORY":
            if "reflective_load" in payload_str or "0x90" in payload_str:
                patterns.append({"header": "SHELLCODE_STUB", "offset": "0x00000450", "risk": "CRITICAL"})
            if "MZ" in payload_str or "svchost" in payload_str:
                patterns.append({"header": "PE_HEADER_IN_MEM", "offset": "0x00400000", "risk": "HIGH"})
        ...
        if len(payload_str) > chunk_size:
            pages = [payload_str[i:i+chunk_size] for i in range(0, len(payload_str), chunk_size)]
            storage_mode = "PAGED"
        else:
            pages = [payload_str]
        ...
        if patterns: summary = f"Potential threat headers found: {', '.join([p['header'] for p in patterns])}."
        ...
        logger.info(f"COLLECTED {artifact_type} for {case_id} [IQ:{len(patterns)} PATTERNS] [SQ:{storage_mode}]")
    ...
    def _generate_simulated_data(self, atype: str, ip: str) -> Any:
        ...
        if atype == "REGISTRY":
            ...
            return [
                {"timestamp": "10:01:22", "proto": "TCP", "src": ip, "dst": "203.0.113.45", "len": 1024, "flag": "PSH,ACK"},
                {"timestamp": "10:02:22", "proto": "TCP", "src": ip, "dst": "203.0.113.45", "len": 128, "flag": "PSH,ACK"},
                {"timestamp": "10:03:22", "proto": "TCP", "src": ip, "dst": "203.0.113.45", "len": 512, "flag": "PSH,ACK"}
            ]
        else:
            return {"detail": f"Raw log fragment from host {ip} collected for manual analysis."}
